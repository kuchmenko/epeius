...
func slipstreamPath(route *quotev1.RouteQuote) ([]byte, error) {
	...
	for i, leg := range route.Legs {
		...
		if !ok || selector.TickSpacing < -8388608 || selector.TickSpacing > 8388607 || !validAddress(leg.TokenIn) || !validAddress(leg.TokenOut) || i > 0 && !strings.EqualFold(route.Legs[i-1].TokenOut, leg.TokenIn) {
			return nil, errors.New("invalid path")
		}
		value := uint32(selector.TickSpacing) & 0xffffff
		path = append(path, common.HexToAddress(leg.TokenIn).Bytes()...)
