...
Atomic V1 is additive and does not replace direct routes or the configured `Executor`. It prepares either one already-returned pinned Uniswap V3 route with one or two legs, or exactly two already-returned direct Uniswap V3 routes with explicit positive allocations. It does not discover or search for split candidates. Two allocations must sum exactly to the quoted input and are re-quoted at that exact input against the original shared EIP-1898 block. Every operation uses a globally distinct physical pool identified by unordered token pair and fee. The engine builds one exact ordered plan, verifies the configured ExecutorV2 runtime, version, and router plus every factory-derived pool identity at the execution block, and simulates the exact unsigned transaction. Recheck does not rebuild the plan.

ExecutorV2 admits only one branch with one or two operations, or two branches with one direct operation each, and only kind 1. Its operation tuple is already the final `(kind, tokenOut, fee, tickSpacing, poolId)` form; inactive fields must be zero. The immutable SwapRouter02 derives each Uniswap V3 pool from its factory, current token pair, and fee. ExecutorV2 pulls exact total input once. Each branch spends its literal allocation and enforces its own positive minimum. For a two-operation route, operation 0 uses router minimum 1 and operation 1 consumes its complete positive measured output above entry dust; operation 1 uses branch minimum. For two branches, each output is measured above the running output balance and their checked sum must equal the final output delta. Each temporary allowance is cleared. Input, intermediate, and output entry balances are restored after final payout. Aggregate minimum is derived independently from combined quoted output under the same slippage rule; it is not replaced by the sum of branch minima. Any failure rolls back the whole call. Events contain each measured operation followed by its branch event, then the plan event.
...
The cross-language fixtures are [`contracts/fixtures/atomic-v1-plan.json`](../contracts/fixtures/atomic-v1-plan.json), [`contracts/fixtures/atomic-v1-two-hop-plan.json`](../contracts/fixtures/atomic-v1-two-hop-plan.json), [`contracts/fixtures/atomic-v1-split-plan.json`](../contracts/fixtures/atomic-v1-split-plan.json), and [`contracts/fixtures/atomic-v1-candidate.json`](../contracts/fixtures/atomic-v1-candidate.json). Their final-interface calldata and distinct accepted-plan, executor-plan, transaction, or read-only candidate commitments were built directly from ABI words with Foundry `cast` and compared with independent Go and TypeScript encoders. Local Solidity, Go, TypeScript, transport, and simulation-fake tests are not deployment evidence. Checked-in TOML has no ExecutorV2 address and keeps execution disabled.
