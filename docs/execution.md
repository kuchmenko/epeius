...
Atomic V1 is additive and does not replace direct routes or the configured `Executor`. It prepares either one already-returned pinned homogeneous Uniswap V3, Pancake V3, or Aerodrome Slipstream Initial route with one or two legs, or exactly two already-returned direct Uniswap V3 routes with explicit positive allocations through the legacy Atomic preparation path. The Atomic plan service does not expose split or mixed-provider candidates. It does not discover or search for split candidates. Two allocations must sum exactly to the quoted input and are re-quoted at that exact input against the original shared EIP-1898 block. Every operation uses a globally distinct exact pool address and physical key identified by provider kind, unordered token pair, and fee or tick spacing. The engine builds one exact ordered plan, verifies the configured ExecutorV2 runtime, version, all immutable router getters, and every factory-derived pool identity at the execution block, and simulates the exact unsigned transaction. Recheck does not rebuild the plan.

ExecutorV2 admits only one branch with one or two operations, or two branches with one direct operation each. Its operation tuple is the final `(kind, tokenOut, fee, tickSpacing, poolId)` form. Kind 1 calls immutable Uniswap SwapRouter02; kind 2 calls immutable Pancake V3 SwapRouter with its exact deadline-bearing tuple. Both require `tickSpacing=0` and `poolId=0` and use the fee. Kind 3 calls the immutable Slipstream Initial router with its deadline-bearing tuple, requires `fee=0`, positive signed `int24 tickSpacing`, and `poolId=0`. All calls send zero value and enforce the positive signed-int256 router input bound. ExecutorV2 pulls exact total input once. Each branch spends its literal allocation and enforces its own positive minimum. For a two-operation route, operation 0 uses router minimum 1 and operation 1 consumes its complete positive measured output above entry dust; operation 1 uses branch minimum. For two branches, each output is measured above the running output balance and their checked sum must equal the final output delta. Each temporary allowance is cleared. Input, intermediate, and output entry balances are restored after final payout. Aggregate minimum is derived independently from combined quoted output under the same slippage rule; it is not replaced by the sum of branch minima. Any failure rolls back the whole call. Events contain each measured operation followed by its branch event, optional `NativeRefunded` for positive Slipstream native delta, then the plan event.
...
After separately verifying and deploying the exact artifact, configure it with at least one matching V3 deployment. The names are independently optional:
