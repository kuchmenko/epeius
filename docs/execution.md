...
Use `--route-id ROUTE_ID --execution-mode atomic-v1`. The route must have exactly one leg and belong to the configured deployment. The terminal independently reads the configured executor runtime, checks its hash and every generic plan field, reconstructs `execute(Plan)` calldata, and recalculates three identities. `plan_id` commits all accepted economic terms. `executor_plan_hash = keccak256(abi.encode(2, chainId, executor, caller, plan))` remains the narrower hash emitted by ExecutorV2. `transaction_fingerprint` binds the plan ID to the exact unsigned transaction, including calldata and gas limit. Canonical receipt success additionally requires the saved executor hash to match `OperationExecuted`, `BranchExecuted`, and `PlanExecuted` in order, exact wallet input, minimum output, and zero net executor/router residue.

The cross-language fixture is [`contracts/fixtures/atomic-v1-plan.json`](../contracts/fixtures/atomic-v1-plan.json). Its final-interface calldata, accepted branch hash, plan ID, transaction fingerprint, and executor plan hash were built directly from ABI words with Foundry `cast` and compared with the independent Go and TypeScript encoders. Local Solidity, Go, TypeScript, and simulation-fake tests are not deployment evidence. The checked-in TOML has no ExecutorV2 address and keeps execution disabled.
