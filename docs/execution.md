...
Before confirmation and again before submission, the terminal checks the route against its local TOML token, deployment, target, and selector or pool-ID lists. It independently encodes expected swap calldata from the displayed route, recipient, input, minimum output, and deadline, and requires an exact byte match. Uniswap permits one exact-input call inside its deadline multicall; Pancake and Slipstream permit their deadline-bearing exact-input calls. Balancer permits one `Vault.swap` with `GIVEN_IN`, empty user data, external balances, and signer as sender and recipient. ERC-20 approval must target the local input token and authorize only the declared amount to the configured spender: router for direct V3, Vault for Balancer, executor for allocations, or Permit2 for Uniswap V4. V4 may then require a separate Permit2 permission for the configured Universal Router. An engine response alone cannot authorize a different target or operation. The local config itself must be trusted.
...
## Atomic V1 local slice

Atomic V1 is additive and does not replace direct routes or the configured `Executor`. It prepares either one already-returned pinned homogeneous Uniswap V3, Pancake V3, or Aerodrome Slipstream Initial route with one or two legs, one direct configured Balancer V2 pool, or exactly two already-returned direct Uniswap V3 routes with explicit positive allocations through the legacy Atomic preparation path. Balancer uses ExecutorV2 kind 4 with zero fee and tick spacing and preserves the exact full `bytes32` pool ID; two-hop, split, and mixed-provider Balancer candidates are unsupported. The Atomic plan service does not expose split or mixed-provider candidates. It does not discover or search for split candidates. Two allocations must sum exactly to the quoted input and are re-quoted at that exact input against the original shared EIP-1898 block. Every operation uses a globally distinct exact pool address or pool ID and physical key identified by provider kind, unordered token pair, and fee, tick spacing, or Balancer pool ID. The engine builds one exact ordered plan, verifies the configured ExecutorV2 runtime, version, immutable provider endpoints and pool list, and every pool identity at the execution block, and simulates the exact unsigned transaction. Recheck does not rebuild the plan.

ExecutorV2 admits only one branch with one or two operations, or two branches with one direct operation each; kind 4 is further restricted to one branch with one operation. Its operation tuple is the final `(kind, tokenOut, fee, tickSpacing, poolId)` form. Kind 1 calls immutable Uniswap SwapRouter02; kind 2 calls immutable Pancake V3 SwapRouter with its exact deadline-bearing tuple. Both require `tickSpacing=0` and `poolId=0` and use the fee. Kind 3 calls the immutable Slipstream Initial router with its deadline-bearing tuple, requires `fee=0`, positive signed `int24 tickSpacing`, and `poolId=0`. Kind 4 calls the immutable Balancer Vault, requires zero fee and tick spacing, and uses an allowed nonzero full pool ID. All calls send zero value. Kinds 1–3 enforce the positive signed-int256 router input bound; kind 4 rejects `uint256.max` so its temporary allowance is always finite. ExecutorV2 pulls exact total input once. Each branch spends its literal allocation and enforces its own positive minimum. For a two-operation route, operation 0 uses router minimum 1 and operation 1 consumes its complete positive measured output above entry dust; operation 1 uses branch minimum. For two branches, each output is measured above the running output balance and their checked sum must equal the final output delta. Each temporary allowance is cleared. Input, intermediate, and output entry balances are restored after final payout. Aggregate minimum is derived independently from combined quoted output under the same slippage rule; it is not replaced by the sum of branch minima. Any failure rolls back the whole call. Events contain each measured operation followed by its branch event, optional `NativeRefunded` for positive Slipstream native delta, then the plan event.

`execute` stays nonpayable. ExecutorV2 accepts empty-calldata native transfers only from the configured Slipstream router while its call is active, snapshots pre-existing native balance, and returns only the positive new delta to the outer caller while the reentrancy guard remains held. Failed delivery or any failure to restore entry native and token balances reverts the complete execution.

After separately verifying and deploying the exact artifact, configure it with at least one matching provider deployment. The names are independently optional:
...
# balancer_deployment = "balancer-v2"
# uniswap_v4_deployment = "uniswap-v4"
```

Use `--route-id ROUTE_ID --execution-mode atomic-v1` for one route, or `--allocations '[{"routeId":"ROUTE_A","amountInAtomic":"13"},{"routeId":"ROUTE_B","amountInAtomic":"24"}]' --execution-mode atomic-v1` for two direct routes. The terminal independently reads the configured executor runtime, checks its hash and every generic plan field, reconstructs `execute(Plan)` calldata, and recalculates three identities. `plan_id` commits accepted economic terms and ordered branch and operation hashes. `executor_plan_hash = keccak256(abi.encode(2, chainId, executor, caller, plan))` remains the narrower hash emitted by ExecutorV2. `transaction_fingerprint` binds the plan ID to the exact unsigned transaction, including calldata and gas limit. Branch reorder changes all three identities and calldata. Canonical receipt success requires exact operation and branch cardinality, order, indices, measured amounts, plan totals, exact wallet input, aggregate minimum output, and zero net executor, router, and transient Permit2 residue. Balancer Vault and Uniswap V4 PoolManager reserve changes are not treated as residue.

The cross-language fixtures are [`contracts/fixtures/atomic-v1-plan.json`](../contracts/fixtures/atomic-v1-plan.json), [`contracts/fixtures/atomic-v1-two-hop-plan.json`](../contracts/fixtures/atomic-v1-two-hop-plan.json), [`contracts/fixtures/atomic-v1-split-plan.json`](../contracts/fixtures/atomic-v1-split-plan.json), [`contracts/fixtures/atomic-v1-candidate.json`](../contracts/fixtures/atomic-v1-candidate.json), [`contracts/fixtures/atomic-v1-pancake.json`](../contracts/fixtures/atomic-v1-pancake.json), [`contracts/fixtures/atomic-v1-slipstream.json`](../contracts/fixtures/atomic-v1-slipstream.json), [`contracts/fixtures/atomic-v1-balancer.json`](../contracts/fixtures/atomic-v1-balancer.json), and [`contracts/fixtures/atomic-v1-uniswap-v4.json`](../contracts/fixtures/atomic-v1-uniswap-v4.json). Their final-interface calldata and distinct provider, operation, candidate, accepted-plan, executor-plan, and transaction commitments were built directly from ABI words with Foundry `cast` and compared with independent Solidity, Go, and TypeScript encoders. Local Solidity, Go, TypeScript, transport, and simulation-fake tests are not deployment evidence. Checked-in TOML has no ExecutorV2 address and keeps execution disabled.
