...
Atomic V1 is additive and does not replace direct routes or the configured `Executor`. It prepares one already-returned pinned Uniswap V3 route with one or two legs as one branch containing the matching generic tagged operations. It does not search for routes. Legs must be token-continuous and use distinct pools. The engine keeps the route's original EIP-1898 quote block, builds one exact plan, verifies the configured ExecutorV2 runtime, version, and router plus every factory-derived pool identity at the execution block, and simulates the exact unsigned transaction. Recheck does not rebuild the plan.

ExecutorV2 admits only that one-branch, one-or-two-operation shape and kind 1 in this slice. Its operation tuple is already the final `(kind, tokenOut, fee, tickSpacing, poolId)` form; inactive fields must be zero. The immutable SwapRouter02 derives each Uniswap V3 pool from its factory, current token pair, and fee. ExecutorV2 pulls exact input once. For a two-operation route, operation 0 uses router minimum 1 and operation 1 consumes its complete positive measured output above entry dust; operation 1 uses branch minimum. Each temporary allowance is cleared. Input, intermediate, and output entry balances are restored after final payout. Independent aggregate minimum remains enforced. Noncanonical ABI bodies fail, and events contain each measured operation in exact order followed by branch and plan events.
...
Use `--route-id ROUTE_ID --execution-mode atomic-v1`. The returned route must have one or two Uniswap V3 legs and belong to the configured deployment. The terminal independently reads the configured executor runtime, checks its hash and every generic plan field, reconstructs `execute(Plan)` calldata, and recalculates three identities. `plan_id` commits all accepted economic terms and ordered operation hashes. `executor_plan_hash = keccak256(abi.encode(2, chainId, executor, caller, plan))` remains the narrower hash emitted by ExecutorV2. `transaction_fingerprint` binds the plan ID to the exact unsigned transaction, including calldata and gas limit. Canonical receipt success requires exact operation cardinality and order, measured operation 1 input equal to operation 0 output, then branch and plan events, exact wallet input, minimum output, and zero net executor/router residue.

The cross-language fixtures are [`contracts/fixtures/atomic-v1-plan.json`](../contracts/fixtures/atomic-v1-plan.json) and [`contracts/fixtures/atomic-v1-two-hop-plan.json`](../contracts/fixtures/atomic-v1-two-hop-plan.json). Their final-interface calldata, accepted operation and branch hashes, plan IDs, transaction fingerprints, and executor plan hashes were built directly from ABI words with Foundry `cast` and compared with independent Go and TypeScript encoders. Local Solidity, Go, TypeScript, and simulation-fake tests are not deployment evidence. Checked-in TOML has no ExecutorV2 address and keeps execution disabled.
