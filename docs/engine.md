...
Adding a positive chain ID enables connectivity checks only. Quotes also require explicit TOML token and deployment allowlists. Uniswap and Pancake deployments use `kind = "uniswap-v3"` or `kind = "pancake-v3"` plus TOML-provided `factory`, `quoter`, `router`, and `fees`. Aerodrome Slipstream uses the same common deployment fields and places `tick_spacings` under the nested deployment `options` table. Balancer V2 uses no common V3 fields and places its `vault` and full `pool` IDs under `options`. Uniswap V4 uses `kind = "uniswap-v4"` with provider-owned contract, router-hash, and pool options described in the [Uniswap V4 provider guide](providers/uniswap-v4.md). Each provider rejects fields owned by another provider. There are no hidden provider chain IDs. Only V4 has an exact-address policy: each reviewed router runtime hash is bound to canonical Permit2. Other compatible configured deployments may be admitted after their provider-specific startup checks. These kinds select supported implementations; they do not allow arbitrary ABIs or calldata. Tokens must remain unique and valid. V3 fees must be unique and range from 0 through 999,999. Slipstream spacings must be unique signed `int24` values. See the [Slipstream](providers/aerodrome-slipstream.md), [Balancer V2](providers/balancer-v2.md), and [Uniswap V4](providers/uniswap-v4.md) provider guides.
...
The separate Atomic V1 plan service reuses only the canonical direct and configured-base-token two-hop iterator for each enabled Uniswap V3 or Pancake V3 deployment named by `atomic_executor`. It records every sequential hop output and emits one full-input homogeneous branch per candidate in stable configured-deployment and path order, including partial results. Candidate IDs use the final typed provider, operation, program, and quote commitments. Detached candidates are stored under their random quote ID for exact `PreparePlan` selection. The service does not select a winner, estimate network cost, discover splits, emit mixed-provider paths, or alter the legacy quote service.
