...
The separate read-only Atomic V1 quote service reuses only the canonical direct and configured-base-token two-hop iterator for the Uniswap V3 deployment named by `atomic_executor`. It records every sequential hop output and emits one full-input branch per candidate in the same stable order, including partial results. Candidate IDs use the final typed provider, operation, program, and quote commitments. It does not store candidates for preparation, select a winner, estimate network cost, discover splits, or alter the legacy quote service.
