...
Aerodrome Slipstream Initial routes display `tick spacing`, not a fee. Their configured SwapRouter is both target and approval spender. Signers with a nonzero Slipstream `tx.origin` fee discount are rejected before READY because informational quotes are not yet account-aware.
