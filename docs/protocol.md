...
`epeius.atomic.v1.AtomicPlanService.GetPlanQuote` is an additive quote path. Its request uses present format version 1, 32-byte unsigned chain ID and input amount, 20-byte token addresses, and a positive search budget. The response has a random 32-byte quote ID, explicit `searchComplete`, and ordered zero or more candidates. This slice returns only one full-input homogeneous branch with one or two sequential Uniswap V3 kind-1, Pancake V3 kind-2, or Aerodrome Slipstream Initial kind-3 operations from enabled Atomic V1 deployments. Slipstream's typed pool carries factory, router, exact pool, and present signed `int24` tick spacing; executable candidates require positive configured spacing. It does not discover split branches, emit mixed-provider candidates, rank candidates, or populate `networkCostOut`.

Each candidate carries the complete typed program, one exact positive output per operation, one pinned block, and a 32-byte candidate ID. The ID commits ordered provider kind and exact factory/router/pool/provider selector, operation, program branch, program, and candidate-quote branch hashes under their `Epeius.Atomic*.v1` domains. Kind 3 hashes its selector as `int24`, not as a V3 fee. Labels, quoter address, latency, and diagnostics do not enter this identity. `PreparePlan` accepts an explicit stored candidate without requoting or substitution; existing quote and preparation APIs remain unchanged.
