...
Atomic V1 responses contain the selected pinned `route` and an `epeius.atomic.v1.Plan`. The plan has one branch and one generic tagged operation; policy currently admits only Uniswap V3 kind 1. Its `plan_id` commits the accepted program, exact executor version and runtime hash, signer and recipient, amounts and minima, quote block, expiry, and deadline. Its separate `executor_plan_hash` remains the contract event commitment over version 2, chain ID, ExecutorV2 address, sender, and the canonical ExecutorV2 `Plan` tuple. `transaction_fingerprint` binds `plan_id` to the exact unsigned chain, sender, target, value, calldata hash, and gas limit. The terminal independently recomputes all three and preserves them unchanged on recheck. Older terminals ignore the additive fields but reject the unfamiliar ExecutorV2 target/calldata as a direct route, so they fail closed.
