...
- `ETHEREUM_ARCHIVE_RPC_URL` enables Balancer's six exact block-19,000,000 quote vectors and on-chain pool identity checks. Without it, that integration test skips while deterministic unit vectors still run.
- `BALANCER_FORK_E2E=1` enables the Balancer Anvil test and uses `ETHEREUM_RPC_URL` as its fork source. It impersonates a known holder only inside the fork, creates a disposable local keystore, exercises approval, fresh quote, test-only snapshot simulation, terminal signing/submission, and canonical receipt checks, then destroys all local state. Production engine configuration remains Tenderly-only.
