...
test("execution config rereads independently and validates executor only for allocations", async () => {
  const directory = await mkdtemp(join(tmpdir(), "epeius-config-"));
  ...
  try {
    ...
    expect(
      ...
      trusted: {
        deployments: {
          uni: { router: `0x${"a".repeat(40)}`, fees: [0, 999999] },
        },
      },
    });
    await expect(
